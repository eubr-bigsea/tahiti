...
def _insert_operation(conn):
    ...
    columns = [c.name for c in tb.columns]
    data = [
      [BASE_OP + 1, 'sentimentanalysis', True, 'TRANSFORMATION', '', '', ''],
    ]
    rows = [dict(zip(columns, row)) for row in data]
    op.bulk_insert(tb, rows)
...
def _insert_operation_translation(conn):
    ...
    columns = [c.name for c in tb.columns]
    data = [
      [BASE_OP + 1, 'pt', 'Análise de Sentimento', 'A Análise de Sentimento verifica para cada declaração se o sentimento é positivo ou negativo'],
      [BASE_OP + 1, 'en', 'Sentiment Analysis', 'Sentiment Analysis checks for every statement if the sentiment is either postive or negative'],
    ]
    rows = [dict(zip(columns, row)) for row in data]
    op.bulk_insert(tb, rows)
...
def _insert_operation_script(conn):
    ...
    columns = [c.name for c in tb.columns]
    data = [
      [BASE_OP + 1, 'JS_CLIENT', 1, "copyInputAddAttributesSplitAlias(task, 'attributes', 'alias', '_sentimentanalysis');", BASE_OP + 1],
    ]
    rows = [dict(zip(columns, row)) for row in data]
    op.bulk_insert(tb, rows)
