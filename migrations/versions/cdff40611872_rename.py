...
def _insert_operation_translation(conn):
    ...
    columns = [c.name for c in tb.columns]
    data = [
      [BASE_OP + 1, 'pt', 'Renomeia os atributos', 'Renomeia os atributos'],
      [BASE_OP + 1, 'en', 'Renames attributes', 'Renames attributes'],
    ]
    rows = [dict(zip(columns, row)) for row in data]
    op.bulk_insert(tb, rows)
...
def _insert_operation_script(conn):
    ...
    columns = [c.name for c in tb.columns]
    data = [
      [BASE_OP + 1, 'JS_CLIENT', 1, "copyInputAddAttributesSplitAlias(task, 'attributes', 'alias', '_renamed');", BASE_OP + 1],
    ]
    rows = [dict(zip(columns, row)) for row in data]
    op.bulk_insert(tb, rows)
...
# nao necessaria para aparecer a operacao
