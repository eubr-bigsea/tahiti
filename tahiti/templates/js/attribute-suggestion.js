...
var TahitiAttributeSuggester = (function () {
	...
	var aggregate = function(task, field, alias){
		task.uiPorts.output = (task.forms.attributes? (task.forms.attributes.value || []): []).slice();
		Array.prototype.push.apply(task.uiPorts.output,
				(task.forms.function? (task.forms.function.value || []) : []).map(function(v){
					return v.alias}));
	}
