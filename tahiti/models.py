...
class OperationType:
    ...
    def values():
        return [
            n
            for n in list(OperationType.__dict__.keys())
...
class DiagramEnvironment:
    ...
    def values():
        return [
            n
            for n in list(DiagramEnvironment.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class ScriptType:
    ...
    def values():
        return [
            n
            for n in list(ScriptType.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class OperationPortType:
    ...
    def values():
        return [
            n
            for n in list(OperationPortType.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class OperationPortMultiplicity:
    ...
    def values():
        return [
            n
            for n in list(OperationPortMultiplicity.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class ApplicationType:
    ...
    def values():
        return [
            n
            for n in list(ApplicationType.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class OperationFieldScope:
    ...
    def values():
        return [
            n
            for n in list(OperationFieldScope.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class DataType:
    ...
    def values():
        return [
            n
            for n in list(DataType.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class PermissionType:
    ...
    def values():
        return [
            n
            for n in list(PermissionType.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class WorkflowType:
    ...
    def values():
        return [
            n
            for n in list(WorkflowType.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class PublishingStatus:
    ...
    def values():
        return [
            n
            for n in list(PublishingStatus.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class TaskGroupType:
    ...
    def values():
        return [
            n
            for n in list(TaskGroupType.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class PluginStatus:
    ...
    def values():
        return [
            n
            for n in list(PluginStatus.__dict__.keys())
            if n[0] != "_" and n != "values"
        ]
...
class Platform(db.Model, Translatable):
    ...
    version = Column(String(200), default=1.0)
    plugin = Column(Boolean, default=False, nullable=False)
...
class OperationSubset(db.Model):
    ...
    platform_id = Column(
        Integer,
        ForeignKey("platform.id", name="fk_operation_subset_platform_id"),
        nullable=False,
        index=True,
    )
...
class RoleOperationSubset(db.Model):
    ...
    subset_id = Column(
        Integer,
        ForeignKey(
            "operation_subset.id", name="fk_role_operation_subset_subset_id"
        ),
        nullable=False,
        index=True,
    )
    subset = relationship(
        ...
        backref=backref("roles", cascade="all, delete-orphan"),
    )
...
class Operation(db.Model, Translatable):
    ...
    enabled = Column(Boolean, nullable=False)
    type = Column(
        Enum(*list(OperationType.values()), name="OperationTypeEnumType"),
        nullable=False,
    )
    icon = Column(String(200), nullable=False)
...
class OperationPort(db.Model, Translatable):
    ...
    type = Column(
        Enum(
            *list(OperationPortType.values()), name="OperationPortTypeEnumType"
        ),
        nullable=False,
    )
    ...
    multiplicity = Column(
        Enum(
            *list(OperationPortMultiplicity.values()),
    ...
    operation_id = Column(
        Integer,
        ForeignKey("operation.id", name="fk_operation_port_operation_id"),
        nullable=False,
        index=True,
    )
    operation = relationship(
        "Operation", overlaps="ports", foreign_keys=[operation_id]
    )
...
class OperationCategory(db.Model, Translatable):
    ...
    default_order = Column(Integer, default=1, nullable=False)
...
class OperationForm(db.Model, Translatable):
    ...
    # Associations
    fields = relationship(
        "OperationFormField", order_by="OperationFormField.order"
    )
...
class Application(db.Model):
    ...
    enabled = Column(Boolean, default=True, nullable=False)
    type = Column(
        Enum(*list(ApplicationType.values()), name="ApplicationTypeEnumType"),
        nullable=False,
    )
...
class OperationScript(db.Model):
    ...
    id = Column(Integer, primary_key=True)
    type = Column(
        Enum(*list(ScriptType.values()), name="ScriptTypeEnumType"),
        nullable=False,
    )
    enabled = Column(Boolean, nullable=False)
    ...
    operation_id = Column(
        Integer,
        ForeignKey("operation.id", name="fk_operation_script_operation_id"),
        nullable=False,
        index=True,
    )
    operation = relationship(
        "Operation", overlaps="scripts", foreign_keys=[operation_id]
    )
...
class PlatformPlugin(db.Model):
    ...
    status = Column(
        Enum(*list(PluginStatus.values()), name="PluginStatusEnumType")
    )
    ...
    use_compiler = Column(Boolean, default=False, nullable=False)
    use_executor = Column(Boolean, default=False, nullable=False)

    # Associations
    platform_id = Column(
        Integer,
        ForeignKey("platform.id", name="fk_platform_plugin_platform_id"),
        nullable=False,
        index=True,
    )
    platform = relationship(
        ...
        overlaps="plugins",
        foreign_keys=[platform_id],
        backref=backref("plugins", cascade="all, delete-orphan"),
    )
...
class OperationFormField(db.Model, Translatable):
    ...
    name = Column(String(200), nullable=False)
    type = Column(
        Enum(*list(DataType.values()), name="DataTypeEnumType"), nullable=False
    )
    ...
    scope = Column(
        Enum(
            *list(OperationFieldScope.values()),
    ...
    form_id = Column(
        Integer,
        ForeignKey("operation_form.id", name="fk_operation_form_field_form_id"),
        index=True,
    )
    form = relationship(
        "OperationForm", overlaps="fields", foreign_keys=[form_id]
    )
...
class Workflow(db.Model):
    ...
    deployment_enabled = Column(Boolean, default=False, nullable=False)
    publishing_enabled = Column(Boolean, default=False, nullable=False)
    publishing_status = Column(
        Enum(*list(PublishingStatus.values()), name="PublishingStatusEnumType")
    )
    type = Column(
        Enum(*list(WorkflowType.values()), name="WorkflowTypeEnumType"),
        default=WorkflowType.WORKFLOW,
        nullable=False,
    )
    preferred_cluster_id = Column(Integer)
    __mapper_args__ = {
    ...
    platform_id = Column(
        Integer,
        ForeignKey("platform.id", name="fk_workflow_platform_id"),
        nullable=False,
        index=True,
    )
    platform = relationship(
        "Platform", overlaps="platform", foreign_keys=[platform_id]
    )
    subset_id = Column(
        Integer,
        ForeignKey("operation_subset.id", name="fk_workflow_subset_id"),
        index=True,
    )
    subset = relationship(
        "OperationSubset", overlaps="subset", foreign_keys=[subset_id]
    )
    pipeline_id = Column(
        Integer,
        ForeignKey("pipeline.id", name="fk_workflow_pipeline_id"),
        index=True,
    )
    pipeline = relationship(
        "Pipeline", overlaps="pipeline", foreign_keys=[pipeline_id]
    )
    platform_id = Column(
        Integer,
        ForeignKey("platform.id", name="fk_workflow_platform_id"),
...
class WorkflowVariable(db.Model):
    ...
    workflow_id = Column(
        Integer,
        ForeignKey("workflow.id", name="fk_workflow_variable_workflow_id"),
        nullable=False,
        index=True,
    )
...
class WorkflowHistory(db.Model):
    ...
    workflow_id = Column(
        Integer,
        ForeignKey("workflow.id", name="fk_workflow_history_workflow_id"),
        nullable=False,
        index=True,
    )
    workflow = relationship(
...
class WorkflowPermission(db.Model):
    ...
    workflow_id = Column(
        Integer,
        ForeignKey("workflow.id", name="fk_workflow_permission_workflow_id"),
        nullable=False,
        index=True,
    )
    workflow = relationship(
...
class Flow(db.Model):
    ...
    environment = Column(
        Enum(
            *list(DiagramEnvironment.values()), name="DiagramEnvironmentEnumType"
        ),
        default=DiagramEnvironment.DESIGN,
        nullable=False,
    )
    ...
    source_id = Column(
        String(250),
        ForeignKey("task.id", name="fk_flow_source_id"),
        nullable=False,
        index=True,
    )
    source = relationship(
    ...
    target_id = Column(
        String(250),
        ForeignKey("task.id", name="fk_flow_target_id"),
        nullable=False,
        index=True,
    )
    target = relationship(
    ...
    workflow_id = Column(
        Integer,
        ForeignKey("workflow.id", name="fk_flow_workflow_id"),
        nullable=False,
        index=True,
    )
    workflow = relationship(
        ...
        overlaps="flows",
        foreign_keys=[workflow_id],
        backref=backref("flows", cascade="all, delete-orphan"),
    )
...
class Task(db.Model):
    ...
    # Fields
    id = Column(String(250), primary_key=True, autoincrement=False)
    ...
    workflow_id = Column(
        Integer,
        ForeignKey("workflow.id", name="fk_task_workflow_id"),
        nullable=False,
        index=True,
    )
    workflow = relationship(
    ...
    operation_id = Column(
        Integer,
        ForeignKey("operation.id", name="fk_task_operation_id"),
        nullable=False,
        index=True,
    )
    operation = relationship(
        "Operation", overlaps="operation", foreign_keys=[operation_id]
    )
...
class TaskGroup(db.Model):
    ...
    name = Column(String(200), nullable=False)
    type = Column(
        Enum(*list(TaskGroupType.values()), name="TaskGroupTypeEnumType"),
        nullable=False,
    )
...
class VerticeTypeProperty(db.Model):
    ...
    vertice_type_id = Column(
        Integer,
        ForeignKey(
            "vertice_type.id", name="fk_vertice_type_property_vertice_type_id"
        ),
        index=True,
    )
    vertice_type = relationship(
        "VerticeType", overlaps="properties", foreign_keys=[vertice_type_id]
    )
...
class VerticeType(db.Model):
    ...
    properties = relationship(
        "VerticeTypeProperty", cascade="all, delete-orphan"
    )
    parent_id = Column(
        Integer,
        ForeignKey("vertice_type.id", name="fk_vertice_type_parent_id"),
        index=True,
    )
    parent = relationship(
        "VerticeType", overlaps="parent", foreign_keys=[parent_id], uselist=False
    )
...
class PipelineTemplate(db.Model):
    ...
    # Associations
    steps = relationship("PipelineTemplateStep", cascade="all, delete-orphan")
...
class PipelineTemplateStep(db.Model):
    ...
    template_id = Column(
        Integer,
        ForeignKey(
            "pipeline_template.id", name="fk_pipeline_template_step_template_id"
        ),
        nullable=False,
        index=True,
    )
    template = relationship(
        "PipelineTemplate", overlaps="steps", foreign_keys=[template_id]
    )
...
class PipelineStep(db.Model):
    ...
    pipeline_id = Column(
        Integer,
        ForeignKey("pipeline.id", name="fk_pipeline_step_pipeline_id"),
        nullable=False,
        index=True,
    )
    pipeline = relationship(
        "Pipeline", overlaps="steps", foreign_keys=[pipeline_id]
    )
    workflow_id = Column(
        Integer,
        ForeignKey("workflow.id", name="fk_pipeline_step_workflow_id"),
        index=True,
    )
    workflow = relationship(
        "Workflow", overlaps="workflow", foreign_keys=[workflow_id]
    )
