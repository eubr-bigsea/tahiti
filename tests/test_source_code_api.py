...
def test_source_code_fail_not_authorized(client):
    tests = [
        lambda: client.get('/source-codes'),
        lambda: client.get('/source-codes/1'),
        lambda: client.patch('/source-codes/1'),
    ]
...
def test_source_code_list_success(client):
    ...
    rv = client.get('/source-codes', headers=headers)
...
def test_source_code_get_success(client):
    headers = {'X-Auth-Token': str(client.secret)}
    source_code_id = 1
    rv = client.get(f'/source-codes/{source_code_id}', headers=headers)
    assert 200 == rv.status_code, 'Incorrect status code'
